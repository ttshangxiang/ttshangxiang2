...
let encode = require('./encode').default;
const header = {
    'Accept': '*/*',
        ...
        return new Promise((resolve, reject) => {
            get(`http://music.163.com/song?id=${song_id}`, function (body, code) {
                if (code == -1) {
                    ...
                    let reg = /<title>(.*)网易云音乐<\/title>/, arr = body.match(reg);
                    let reg2 = /class="j-img" data-src="(.*)">/, arr2 = body.match(reg2);
                    let reg3 = /<p class="des s-fc4">歌手：(.*)<\/p>/, arr3 = body.match(reg3);
                    let reg4 = /<p class="des s-fc4">所属专辑：(.*)<\/p>/, arr4 = body.match(reg4);
                    let obj;
                    if (arr[1]) {
